...
function SubmitButton({ text }: { text: string }) {
  ...
  return <button className="btn btn-dark" type="submit" disabled={pending}>{pending ? "Отправляем…" : text}<ArrowUpRight size={17} /></button>;
}

export function LeadForm({ programs, submitText }: { programs: { title: string }[]; submitText: string }) {
  const [state, action] = useActionState(submitLead, initialState);
  return (
    <form action={action} className="form-grid">
      <div className="honeypot" aria-hidden="true"><label htmlFor="website">Сайт</label><input id="website" name="website" tabIndex={-1} autoComplete="off" /></div>
      <div className="field"><label htmlFor="name">Имя родителя</label><input id="name" name="name" autoComplete="name" required placeholder="Как к вам обращаться" aria-invalid={Boolean(state.fieldErrors?.name)} />{state.fieldErrors?.name?.[0] ? <span className="field-error">{state.fieldErrors.name[0]}</span> : null}</div>
      <div className="field"><label htmlFor="phone">Телефон</label><input id="phone" name="phone" type="tel" autoComplete="tel" required placeholder="+7 ___ ___ __ __" aria-invalid={Boolean(state.fieldErrors?.phone)} />{state.fieldErrors?.phone?.[0] ? <span className="field-error">{state.fieldErrors.phone[0]}</span> : null}</div>
      ...
      <div className="field-full"><SubmitButton text={submitText || "Отправить заявку"} /><p className="form-note">Нажимая кнопку, вы соглашаетесь на обработку данных для связи по заявке.</p></div>
    </form>
  );
}
